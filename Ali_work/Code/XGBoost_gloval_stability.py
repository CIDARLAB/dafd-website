...
# -*- coding: utf-8 -*-
"""
Created on Mon Sep 12 13:01:48 2022
...
### Adding the output layer
    ...
    model.add(Dense(units = 8, activation = 'relu', name='scratch_dense_4'))#, kernel_regularizer=regularizers.l2(0.001))) #more options to avoid over-fitting


    model.add(Dense(units = 4, activation = 'relu', name='scratch_dense_5'))#, kernel_regularizer=regularizers.l2(0.001))) #more options to avoid over-fitting
...
# X_test = PolynomialFeatures(degree=3).fit_transform(X_test)
    ...
    reg = xgb.XGBRegressor(tree_method="hist").fit(X_train, Y_train)
...
### Read data
    ...
    y_lit_pred = reg.predict(X_lit) #model.predict(X_lit)
...
#print("Mean squared error (MSE) for train-set:       %f" % sklearn.metrics.mean_squared_error(Y_train, y_pred_train))
    ...
    print("R square (R^2) for RPMI:                 %f" % sklearn.metrics.r2_score(Y_lit, D_lit_pred))
...
print(f' Mean absolute error (MAE) for lit: {np.mean(mael):.2f} +- {1.96 * np.std(mael) / np.sqrt(len(mael)):.2f}')
print(f' Root mean squared error (RMSE) for lit: {np.mean(rmsel):.2f} +- {1.96 * np.std(rmsel) / np.sqrt(len(rmsel)):.2f}')
print(f' R square (R^2) for lit: {np.mean(r2l):.2f} +- {1.96 * np.std(r2l) / np.sqrt(len(r2l)):.2f}')
print(f' Mean absolute percentage error for lit: {np.mean(mapel):.2f} +- {1.96 * np.std(mapel) / np.sqrt(len(mapel)):.2f}')
...
#X_inner = wb.loc[:,['Orifice width (um)','Flow rate ratio','New_ca_number' ]]
...
y_inner_pred = reg.predict(X_inner) 
D_inner_pred=Ori_inner*y_inner_pred
...
print("Mean absolute percentage error size FF1:   %f" % mean_absolute_percentage_error2(Y_inner, D_inner_pred))
...
#X_outer = wb.loc[:,['Orifice width (um)','Flow rate ratio','New_ca_number' ]]
...
y_outer_pred = reg.predict(X_outer) 
...
plt.show()
print("Mean absolute percentage error size FF2:   %f" % mean_absolute_percentage_error2(Y_outer, D_outer_pred))
...
#X_ins1 = wb.loc[:,['Orifice width (um)','Flow rate ratio','New_ca_number' ]]
...
y_ins1_pred = reg.predict(X_ins1) 
...
#X_ins2 = wb.loc[:,['Orifice width (um)','Flow rate ratio','New_ca_number' ]]
...
y_ins2_pred = reg.predict(X_ins2) 
